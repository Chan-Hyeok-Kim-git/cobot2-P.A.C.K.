...
def generate_launch_description():
    ...
    # ------------------------------------------------------------------
    integrated_xacro = os.path.join(bringup_pkg, 'urdf', 'm0609_with_rg2.urdf.xacro')
    robot_description = {
    ...
    # 팔 관절(/dsr01/joint_states) + 그리퍼 관절(/gripper_joint_states)을
    # 하나의 /joint_states로 합친다. robot_state_publisher/MoveIt/RViz는
    # 이 합쳐진 /joint_states만 구독한다.
    joint_state_merger = Node(
        ...
        parameters=[
            robot_description,
            {'source_list': [f'/{ROBOT_NAME}/joint_states', '/gripper_joint_states']},
        ],
        output='screen',
    )
